...
func _init() -> void:
    ...
    assert(int(summary.performance.score) > 0)
    assert(summary.history.size() == 1)
    assert(not summary.relics.is_empty())
    assert(int(integration.unlock_economy.resources.gold) >= int(summary.result.resources.gold))
    assert(integration.tutorial_progress.completed.has("protect_treasure"))
    ...
    assert(restored.run_history.entries.size() == 1)
    assert(not restored.relics.owned.is_empty())
    ...
    assert(not reloaded.last_run_result.is_empty())
    store.delete_save()
